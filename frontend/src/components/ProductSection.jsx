...
function ProductSection() {
  const { allProducts, provinces, loading, error } = useProduct();
  ...
  return (
    <div>
      {/* <ProductFilterSection onViewModeChange={setViewMode} onFilterChange={setFilteredProducts} /> */}
          ...
          <div className={viewMode === "grid" ? "grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" : "space-y-6"}>
            {filteredProducts.map((product) => (
              <ProductCard
                key={product.id}
                product={product}
                currentTexts={{ inStock: "In Stock", outOfStock: "Out of Stock", orderNow: "Order Now", from: "From", orderPlaced: "Order Placed" }}
                currentLanguage="en"
