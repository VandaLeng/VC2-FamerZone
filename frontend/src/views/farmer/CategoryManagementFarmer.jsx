...
const CategoryManagement = ({ currentLanguage = 'en', setCurrentLanguage }) => {
    ...
    const fetchCategories = async () => {
      ...
      try {
        ...
        const transformedCategories = response.data.data.map((category) => ({
          id: category.id,
          name: category.name, // Already { en: '', kh: '' }
          description: category.description, // Already { en: '', kh: '' }
  ...
  const filteredCategories = categories.filter((category) => {
    const matchesSearch =
      category.name[currentLanguage].toLowerCase().includes(searchTerm.toLowerCase()) ||
      category.description[currentLanguage].toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus = filterStatus === 'all' || category.status === filterStatus;
    return matchesSearch && matchesStatus;
  });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      formDataToSend.append('name[kh]', formData.name.kh);
      formDataToSend.append('description[en]', formData.description.en || '');
      formDataToSend.append('description[kh]', formData.description.kh || '');
                ...
                <div>
                  ...
                  {selectedCategory?.image_url && (
                    <img
                      ...
                      alt={selectedCategory.name[currentLanguage]}
